...
def train(local_rank, args, hp, model):
    ...
    save_weights = './weights/transformer'
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % hp.train.accum_grad == 0:
                ...
                if global_step % 5000 == 0:
                    torch.save(model.state_dict(), save_weights + "/transformer_model-{}.pth".format(global_step))
...
def main():
    ...
    model = CNN_Transformer(image_size=hp.data.image_size, patch_size=hp.model.patch_size,
                            num_classes=hp.model.num_classes,
                            dim=hp.model.dim, depth=hp.model.depth, heads=hp.model.heads, pool=hp.model.pool,
                            in_channels=hp.model.in_channels, out_channels=hp.model.out_channels,
                            with_lca=hp.model.with_lca)
...
if __name__ == "__main__":
    ...
    main()
    end_time = time.time()
    print('训练总耗时：' + str(round((start_time - end_time) / 3600, 2)) + '小时！')
